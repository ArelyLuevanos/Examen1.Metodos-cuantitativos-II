...
#Se conprueba entonces cual de los dos modelos es el que más funciona para entender el comportamiento de las variables
...
#para conocer a plenitud el comportamiento de esas variables, incluso estos datos pueden hablar de la actividad económica
#del municipio o estado.

#Para usar tablas y tener un mejor analisis se instala el paquete...

install.packages("rockchalk")
library(rockchalk)

ex1w <- outreg (list("Efectos Fijos" = EFECTOSFIJOS, "Efectos Aleatorios" = EFECTOSALEATORIOS) , 
                title = "Tabla Residuales" , centering="dcolumn",
                gofNames = c("Estimate"="Coeficientes"),
                label = "tab:tabla1" , tight = FALSE , float = TRUE , print.results = F)

cat (ex1w, file="tabla.tex")
